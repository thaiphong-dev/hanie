...
- Name: admin\admin.spec.ts >> 4.3 — POS flow >> tìm khách theo SĐT → thấy tên khách
- Location: tests\e2e\admin\admin.spec.ts:114:7
...
Locator: getByPlaceholder(/sđt|số điện thoại|phone|search.*customer/i).or(getByLabel(/sđt|số điện thoại|phone/i)).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByPlaceholder(/sđt|số điện thoại|phone|search.*customer/i).or(getByLabel(/sđt|số điện thoại|phone/i)).first()
