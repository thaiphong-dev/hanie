...
- Name: auth\auth.spec.ts >> 1.4 — Admin guard >> admin login → /vi/admin → thấy dashboard
- Location: tests\e2e\auth\auth.spec.ts:150:7
...
Locator: getByText(/dashboard|doanh thu|lịch hẹn|overview/i)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/dashboard|doanh thu|lịch hẹn|overview/i)
