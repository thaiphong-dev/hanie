...
export default function InvoiceDetail() {
  ...
  if (loading)
    return (
      <div className="p-10 text-center font-body text-text-muted">
        Đang tải hoá đơn...
      </div>
    );
  if (!order)
    return (
      <div className="p-10 text-center font-body text-red-500">
      ...
      <div className="bg-white rounded-3xl border border-bg-secondary shadow-sm overflow-hidden print:border-0 print:shadow-none">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center">
                <span className="font-display text-lg text-bg-dark font-bold">
                  H
                </span>
              </div>
              <h1 className="font-display text-2xl text-text-inverse tracking-wide">
                Hanie Studio
              </h1>
            </div>
            <div className="space-y-1 font-body text-sm text-text-muted">
              <p>09A Nguyễn Đình Thụ, Quy Nhơn Nam, Gia Lai</p>
              <p>Hotline: 09xx xxx xxx</p>
              <p>Facebook.com/hanie.studio</p>
            </div>
          </div>
          <div className="text-right flex flex-col justify-end">
            <p className="font-display text-3xl text-text-inverse mb-2">
              HOÁ ĐƠN
            </p>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b-2 border-bg-dark/10">
                <th className="py-3 text-left font-body text-xs font-semibold text-text-muted uppercase tracking-wider">
                  Dịch vụ
                </th>
                <th className="py-3 text-center font-body text-xs font-semibold text-text-muted uppercase tracking-wider">
                  SL
                </th>
                <th className="py-3 text-right font-body text-xs font-semibold text-text-muted uppercase tracking-wider">
                  Đơn giá
                </th>
                <th className="py-3 text-right font-body text-xs font-semibold text-text-muted uppercase tracking-wider">
                  Thành tiền
            ...
            <tbody className="divide-y divide-bg-secondary">
              {order.order_items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="py-4 text-center font-body text-sm text-text-secondary">
                    {item.quantity}
                  </td>
                  <td className="py-4 text-right font-body text-sm text-text-secondary">
                    {formatVND(item.price)}
                  </td>
                  <td className="py-4 text-right font-body text-sm font-semibold text-text-primary">
            ...
            {order.discount_amount > 0 && (
              <div className="flex justify-between items-center text-green-600">
                <span className="font-body text-sm flex items-center gap-1.5">
                  Giảm giá{" "}
                  {order.voucher?.code && (
                    <small className="text-[10px] bg-green-100 px-1 rounded">
                      ({order.voucher.code})
                    </small>
                  )}
                </span>
        ...
        <div className="px-8 py-6 border-t border-bg-secondary flex flex-wrap justify-between items-center gap-4 bg-white no-print">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-body text-[10px] text-text-muted uppercase tracking-wide">
                Nhân viên thực hiện
              </p>
              <p className="font-body text-sm font-medium text-text-primary">
