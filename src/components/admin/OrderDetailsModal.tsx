    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto print-area">
          {loading ? (
            <div className="p-20 text-center font-body text-sm text-text-muted">
              Đang tải...
            </div>
          ) : !order ? (
            <div className="p-20 text-center font-body text-sm text-red-500">
            ...
            <div className="p-0 border-0">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center">
                      <span className="font-display text-sm text-bg-dark font-bold">
                        H
                      </span>
                    </div>
                    <h1 className="font-display text-lg text-text-inverse tracking-wide">
                      Hanie Studio
                    </h1>
                  </div>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-bg-secondary">
                      <th className="py-2 text-left font-body text-[10px] font-semibold text-text-muted uppercase tracking-wider">
                        Dịch vụ
                      </th>
                      <th className="py-2 text-center font-body text-[10px] font-semibold text-text-muted uppercase tracking-wider">
                        SL
                      </th>
                      <th className="py-2 text-right font-body text-[10px] font-semibold text-text-muted uppercase tracking-wider">
                        Thành tiền
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-bg-secondary">
                    {order.order_items.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="py-3 text-center font-body text-sm text-text-secondary">
                          {item.quantity}
                        </td>
                        <td className="py-3 text-right font-body text-sm font-semibold text-text-primary">
              ...
              <div className="px-8 py-4 border-t border-bg-secondary flex justify-between items-center bg-white/50 no-print">
                <p className="font-body text-[10px] text-text-muted italic">
                  KTV: {order.staff?.full_name || "Hệ thống"}
                </p>
                <div className="text-right">
                  <p className="font-body text-[9px] text-text-muted uppercase">
                    Hoa hồng dự tính
                  </p>
                  <p className="font-body text-xs font-semibold text-emerald-600">
                    {formatVND(Math.round(order.total * 0.1))}
                  </p>
                </div>
              </div>
            </div>
